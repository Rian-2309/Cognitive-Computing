# Definition of Intelligence
Intelligence is the **ability to learn from experience, adapt to new situations, understand complex ideas, solve problems and use knowledge effectively**.
>**Intelligence = Learning + Problem Solving + Adaptation**
---
# Theories of Intelligence
## 1. Spearman’s Two-Factor Theory
Charles Spearman proposed:
- **g-factor (general intelligence)**
- **s-factor (specific abilities)**
## 2. Thurstone’s Primary Mental Abilities
Thurstone rejected g-factor and proposed **7 abilities**:
- verbal comprehension
- numerical ability
- memory
- reasoning
- perceptual speed
- word fluency
- spatial ability
Thurstone tells intelligence is not single, it is combination of multiple abilities.
## 3. Gardner’s Multiple Intelligence Theory
Proposed by Howard Gardner.
### Types
![[Pasted image 20260317234808.png]]

| Type                 | Explanation          |
| -------------------- | -------------------- |
| Linguistic           | Language skills      |
| Logical-Mathematical | Reasoning and math   |
| Spatial              | Visual thinking      |
| Musical              | Music understanding  |
| Bodily-Kinesthetic   | Physical skills      |
| Interpersonal        | Understanding others |
| Intrapersonal        | Self-awareness       |
| Naturalistic         | Understanding nature |
Later, he also suggested **Existential Intelligence**.
## 4. Sternberg’s Triarchic Theory
Intelligence has **3 types**:

| Type       | Meaning          |
| ---------- | ---------------- |
| Analytical | Problem solving  |
| Creative   | New ideas        |
| Practical  | Real-life skills |

---
# Types of Intelligence
## 1. Fluid vs Crystallized Intelligence
| Type                      | Meaning                    |
| ------------------------- | -------------------------- |
| Fluid Intelligence        | New problems solve karna   |
| Crystallized Intelligence | Experience based knowledge |
## 2. Emotional Intelligence (EQ)
Emotional Intelligence is the ability to recognize, understand and manage emotions in oneself and others.
### Components (Daniel Goleman)
| Component       | Meaning                     |
| --------------- | --------------------------- |
| Self-awareness  | Apni emotions samajhna      |
| Self-regulation | Emotions control karna      |
| Motivation      | Goal-oriented behavior      |
| Empathy         | Dusron ki feelings samajhna |
| Social skills   | Relationships manage karna  |
## 3. Cultural Intelligence
Ability to **adapt to different cultures**.

---
# Intelligence Quotient (IQ)
IQ is a measure of a person's intellectual abilities compared to others.
## IQ Formula
>IQ = (Mental Age / Chronological Age) × 100
## Types of Tests
| Test                | Description                  |
| ------------------- | ---------------------------- |
| Stanford-Binet Test | Individual intelligence test |
| WAIS                | Adult intelligence test      |
| Group tests         | Multiple people test         |
## IQ Classification
| IQ Range | Level                   |
| -------- | ----------------------- |
| 130+     | Very superior           |
| 110–129  | Above average           |
| 90–109   | Average                 |
| 70–89    | Below average           |
| <70      | Intellectual disability |

---
# Factors Affecting Intelligence
## Biological Factors
- genetics
- brain structure
- health
## Environmental Factors
- education
- family
- culture
- nutrition
---
# Creativity (Part of Intelligence)
Creativity is the **ability to generate new, original and useful ideas**.
### Types of Thinking
| Type                | Meaning                 |
| ------------------- | ----------------------- |
| Divergent thinking  | generate Multiple ideas |
| Convergent thinking | choose Best solution    |

---
# Intelligence vs Creativity
| Intelligence     | Creativity          |
| ---------------- | ------------------- |
| Logical thinking | Innovative thinking |
| Problem solving  | Idea generation     |
| Measured by IQ   | Hard to measure     |

---
# Intelligence in the Brain
| **Brain Region**  | **Function**    |
| ----------------- | --------------- |
| Frontal lobe      | Decision making |
| Parietal lobe     | Reasoning       |
| Prefrontal cortex | Planning        |

---
# AI Intelligence Types
| Type       | Description             |
| ---------- | ----------------------- |
| Narrow AI  | Specific tasks          |
| General AI | Human-like intelligence |
| Super AI   | Beyond human (future)   |
## AI Capabilities
- learning (ML)
- reasoning
- decision making
- pattern recognition
---
# Intelligence in Cognitive Computing
Intelligence simulates in AI systems.
### Examples
| Human Intelligence  | AI Equivalent    |
| ------------------- | ---------------- |
| Learning            | Machine Learning |
| Problem solving     | Algorithms       |
| Decision making     | AI models        |
| Pattern recognition | Deep learning    |

---
# Applications of Intelligence
| Field      | Use                  |
| ---------- | -------------------- |
| Education  | Learning improvement |
| Healthcare | Diagnosis            |
| Business   | Decision making      |
| AI         | Automation           |

---
# Limitations of Intelligence
- IQ tests are not fully accurate.
- Creativity did not measure.
- It can be Cultural bias.
- Ignore emotional intelligence